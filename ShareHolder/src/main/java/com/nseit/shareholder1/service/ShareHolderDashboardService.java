...
@Service
@Slf4j
public class ShareHolderDashboardService {
	...
	public ResponseEntity<?> getShareholderAllShareTransferDetails(String version) {
		try {
			...
			if(shareHolderTransferDetails.size() !=0) {
				...
				log.error("In getShareholderAllShareTransferDetails No_DATA--------------------------------");
				return response.getAuthResponse("NO_DATA", HttpStatus.OK, null, version);
			}
			
		} catch (Exception e) {
			log.error("In getShareholderAllShareTransferDetails Exception INTERNAL_SERVER_ERROR--------");
